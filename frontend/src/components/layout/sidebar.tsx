...
import {
  LayoutDashboard, Users, UserPlus, Handshake, GitBranch, CalendarDays,
  Mail, Activity, Megaphone, FileText, Receipt, Package, Warehouse, Warehouse as WarehouseIcon, Truck,
  ShoppingCart, Factory, BadgeDollarSign, UserCog, Clock, CalendarOff,
  FolderKanban, LifeBuoy, BarChart3, FileArchive, Shield, Bell, Settings,
...
const navGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/gatepass", label: "Gatepass", icon: Shield },
...
export default function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed left-0 top-0 bottom-0 z-40 flex flex-col transition-all duration-700 ease-in-out border-r border-white/20 bg-white/40 backdrop-blur-3xl shadow-[4px_0_24px_rgba(0,0,0,0.02)]",
    ...
    >
      ...
      <div className="p-6 border-t border-white/20 bg-white/20 backdrop-blur-sm">
         <div className="flex items-center gap-3 p-3 rounded-2xl bg-white/40 border border-white/60 shadow-sm group cursor-pointer hover:bg-white/60 transition-all duration-500">
            ...
            <button onClick={(e) => { e.preventDefault(); toggleSidebar(); }} className="p-1.5 rounded-lg text-slate-300 hover:text-indigo-600 hover:bg-white/80 transition-all">
